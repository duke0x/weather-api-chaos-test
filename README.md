...
Конфигурация:
   ...
   - Master-нода: 10.0.10.5, pg-0  
   - Slave-нода:  10.0.10.6, pg-1
...
3. Реальные результаты
    ...
    ![net-problems](./img/image.png)

4. Анализ результатов

    Все работает штатно, проблем не выявлено.
...
3. Реальные результаты
    ...
    ![pgdown-alert](./img/image-4.png)
    
    Получил алерт в телеграмм:
    
    ![tele-alert](./img/image-5.png)
    
    Восстановил работу сервиса patroni на pg-0.
    
    ![no-pg-alert](./img/image-6.png)

    Получил уведомление, что алерт закончился:
    
    ![tele-no-alert](./img/image-7.png)
